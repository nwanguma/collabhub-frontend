...
export const ActivityComponent: React.FC<ActivityProps> = ({
  ...
  const { type, owner, participant, project, profile } = activity;
  ...
  const resourceTitle = project?.title;
  const resource = project || profile;
  const resourceType = project ? "projects" : profile ? "profiles" : "";
